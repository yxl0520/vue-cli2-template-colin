  ...
  <div class="axios-demo">
    ...
    <div class="item demo1">
      ...
      <p class="note">
        ...
        【3】并启动server\dev.server.js的Express服务器</p>
      <button @click="axiosOriginFetch">点击加载</button>
      <div class="data-wrapper">{{axiosData1}}</div>
    </div>
    <div class="item demo2">
      <h2 class="sub-title">(2)  【webpack跨域解决方案演示】：pathRewrite使用</h2>
      <p class="note">
        【1】使用封装好的$http；<br>
        【2】请将toggleBaseUrl设置为 4（模拟api地址为/seller）;<br>
        【3】本demo将从Express服务器获取数据，服务器地址为：http://localhost:8900/api/seller，注意：我们需提前启动服务器<br>
        【4】webpack-dev-serve配置含义：本地发起的 http://localhost:8080/mockapi/seller最终会被重定向到正确的服务器地址，即http://localhost:8900/api/seller，同时也解决了跨域问题;<br>
        【5】还需要修改axios的baseURL为'/mockapi'
      </p>
      <button @click="httpFetchFromExpress">点击加载</button>
      <div class="data-wrapper">{{axiosData2}}</div>
    </div>
    <div class="item demo3">
      <h2 class="sub-title">(3) 使用封装方法：$http 获取资源2</h2>
      <p class="note">
        【1】从mockjs获取数据, 请将toggleBaseUrl设置为 3；<br>
        【2】模拟api地址为/mockapi/demo/PersonEntry
      </p>
    ...
    <div class="item demo4">
      ...
      <p class="note">
        【1】演示Mock.js接口拦截数据--http://text.com；<br>
        【2】使用的是原始axios请求方法
      </p>
    ...
    <div class="item demo5">
      ...
      <p class="note">
        【1】演示接口错误的情况；<br>
        【2】从mockjs获取数据, 请将toggleBaseUrl设置为 3
      </p>
  ...
  methods: {
    ...
    httpFetchFromExpress () {
      ...
      this.$http.getData('seller').then(res => {
        console.log('使用基于axios封装的http方法从Express服务器获取数据:', res.data);
